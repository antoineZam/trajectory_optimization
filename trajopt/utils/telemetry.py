...
class RacingTelemetry:
    ...
    def end_episode(
        ...
        time_on_track = (
            sum(1 for w in wheels_inside_counts if w >= 2)
            / len(wheels_inside_counts) * 100
        )
        ...
        summary = EpisodeSummary(
            ...
            average_racing_line_distance=(
                np.mean(racing_line_distances)
                if racing_line_distances else 0.0
            ),
    ...
    def _print_episode_summary(self, summary: EpisodeSummary):
        ...
        print(f"{'='*80}")
        print(
            f"{self.BOLD}EPISODE {summary.episode}"
            f" SUMMARY [{summary.training_phase.upper()}]"
            f"{self.RESET}"
        )
        print(f"{'='*80}")
    ...
    def print_training_progress(self, episodes_completed: int):
        ...
        # Calculate trends
        completion_rate = (
            sum(1 for ep in recent_episodes if ep.lap_completed)
        ...
        if hasattr(self, 'best_lap_time') and self.best_lap_time < float('inf'):
            print(
                f"  {self.GREEN}Best Lap Time:"
                f" {self.best_lap_time:.1f}s"
